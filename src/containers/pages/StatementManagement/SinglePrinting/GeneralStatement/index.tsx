...
const GeneralStatement: FC<GeneralStatementProps> = ({ goBack, accountId }) => {
  const columns = [
    {
  ...
  return (
    <Container
      next={{
        name: `Print All ${selectedTransactions.length}`,
        onClick: confirm,
        disabled: !selectedTransactions.length,
      }}
      previous={{ name: "Back", onClick: goBack }}
    >
      ...
      <Table
        ...
        rowSelection={{
          onChange: (s, ss) => setSelectedTransactions(s),
        }}
        pagination={{
          ...paginationConfig,
          defaultCurrent: 1,
...
interface GeneralStatementProps {
  goBack: () => any;
  accountId: string;
}
...
export default connect(mapStateToProps, mapDispatchToProps)(GeneralStatement);
