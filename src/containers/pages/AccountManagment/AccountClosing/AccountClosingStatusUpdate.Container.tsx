...
export const AccountClosingStatusUpdateListContainer = () => {
...
const AccountHolder = ({ data, charges }: { data: any; charges: any }) => {
  ...
  useSDK(
    async (sdk: LOLCSDK) =>
      sdk.AccountService.getPendingAccountClose().then((data: any) => {
        setStatusList(data);
      }),
    [data],
    false,
  ...
  const onCreateAccount = () => {
    Modal.confirm({
      ...
      content: (
        <>
          <p className="text-s mb-4 text-gray-700">
            Results of this state will Close the account
          </p>
          <p className="text-xs">Withdrawal Amount</p>
          <Input disabled style={{ width: 250 }} />
        </>
      ),
      okText: "Confirm",
      onOk() {
        SDK.AccountService.approveAccountCloseFinalWithdrawal(filteredObj?.id, {
