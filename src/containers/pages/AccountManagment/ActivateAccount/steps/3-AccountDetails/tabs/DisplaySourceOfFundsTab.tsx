...
export const DisplaySourceOfFundsTab = ({ data }: { data: any }) => {
  const cardSchema = ActivateAccountSchema.steps![3]!.cards![3];
  console.log("asdhasdhasdjh🤷‍♀️", data);
  return (
    <FormCardTemplate
      ...
      <>
        <div className='flex mb-4 flex-col'>
          <div className='mb-8'>
            {data
              ? data.SourceOfFunds
                ? data.SourceOfFunds.map((item: any, index: number) => {
                    return (
                      <>
                        <BasicCheckbox
          ...
          <InputContainer
            ...
            input={
              <BasicTextArea
                ...
                className='w-100'
                value={
                  data.SourceOfFunds?.find(
                    (item: any) => item.description === "Other"
                  )?.otherRemarks
                }
              ></BasicTextArea>
            }
